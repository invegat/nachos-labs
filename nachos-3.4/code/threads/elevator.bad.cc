...
//}
Person * ELEVATOR::getPPFirst(Person * p[]) {
    int i = 0;
    while (p[i] != NULL) {
        if (p[i] != (Person *)-1) {
            return p[i];
...
Person * ELEVATOR::removeFromPPFirst(Person * p[]) {
    int i = 0;
    while (p[i] != NULL) {
        if (p[i] != (Person *)-1) {
            Person * fp = p[i];
...
bool ELEVATOR::addToPP(Person *pp[], Person * p) {
    personLock->Acquire();
    for (int i = 0;i<tp;i++) {
        if (pp[i] == NULL)
            break;
...
//             }
            ...
            if (from == currentFloor) {
                ...
                while (o < maxOccupancy) {
                    ...
                    Person * p = removeFromPPFirst(listEntering[currentFloor]);
                    ASSERT(p != NULL);
                    o++;
                }
                from = -1;
...
//                printf("Broadcasting\n");
                ...
                for (int j=0;j<i;j++) {
                    Person * p = removeFromPPFirst(listLeaving[currentFloor]);
                    ASSERT(p != NULL);
                }
...
bool ELEVATOR::hailElevator(Person *p) {
//    printf("hailElevator called with id %d\n", p->id);
    reHail = false;
    sp = NULL;
    IntStatus oldLevel = interrupt->SetLevel(IntOff);	// disable interrupts
...
//    floorLock->Acquire();
    ...
    if (occupancy > 0) {
        if (reHail) {
            p = getPPFirst(personsOn);
            if (!addToPP(listLeaving[p->toFloor], p))
                return false;
        }
...
void PersonThread(int person) {
   ...
   bool reHail = false;
   do {
       reHail = e->hailElevator(p);
       if (reHail) printf("reHailing with id %d", p->id);
   } while(reHail);
   printf("called hailElevator with id %d\n", p->id);
   (void) interrupt->SetLevel(oldLevel);	// re-enable interrupts
}
